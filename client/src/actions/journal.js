...
import {
  ...
  GET_GROUPS_URL,
  RETURN_STUDENT_WORK_URL,
  RETURN_OWN_WORK_URL,
  SUBMIT_WORK_URL,
  UPDATE_TASK_URL,
  UPLOAD_WORK_FILE_URL,
  SET_GRADE_URL,
  DOWNLOAD_WORK_FILE_PATTERN,
} from "../constants";
...
export const getGroupStudents = (groupID) => async (dispatch) => {
  try {
    const response = await $host.get(GET_GROUP_STUDENTS_URL, {
      params: { groupID },
    });
...
export const getNextAdditionID = (additions) => {
  ...
  additions.forEach((addition) => {
    if (addition.id > maxID) maxID = addition.id;
  });
  return maxID + 1;
};
...
export const convertRemoteAdditionsToEditFormat = (additions) => {
  ...
  additions.map((addition) => {
    const name =
      addition.type === ADDITION_TYPE_FILE
