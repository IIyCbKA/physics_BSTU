export const minimizeStr = (string, maxLength, afterPoint) => {
  ...
  if (lengthString > maxLength) {
    ...
    string =
      string.slice(0, maxLength - lengthString + indEndName - spareSpace) +
      "…" +
      string.slice(indEndName, lengthString);
  }
...
export const getLastDirectory = (path) => {
  if (getAfterDiskPath(path) === "") return "";
...
export const getCurrentFolderName = (path) => {
  ...
  if (getAfterDiskPath(path) === "") return defaultDiskTitle;
...
export const getFileType = (filename) => {
  ...
  else return filename.slice(pointPos + 1);
};

export const getFilenameOnly = (filename) => {
  const pointPos = filename.lastIndexOf(".");
  if (pointPos === -1) return filename;
  else return filename.slice(0, pointPos);
};
