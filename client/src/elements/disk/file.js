...
export default function File(props) {
  ...
  useLayoutEffect(() => {
    if (orientation === PORTRAIT_ORIENTATION) {
      minimizeStrPortrait(props.name, nameWidthPortrait, nameFieldRef.current);
    } else {
      nameFieldRef.current.textContent = minimizeStr(
        props.name,
        DEFAULT_LANDSCAPE_SYMBOLS,
  ...
  const handleTouchEnd = (event) => {
    ...
    const isNotMove =
      Math.abs(touchEndX - touchStartX) < 5 &&
  ...
  return (
    <div
      className={`file-area${selected_id === props.id ? " selected" : ""}`}
    ...
    >
      ...
      <div
        className="item-info"
        style={props.type !== FILE_TYPE_FOLDER ? {padding: '8px 0px 4px'} : {}}
      >
