...
export default function FormsHeader(props) {
  ...
  const journalGroupName = useSelector(
    (state) => state.selectedGroup.groupName,
  );
  ...
  const handleCreateTaskClick = async (event) => {
    ...
    if (isUpdated) {
      ...
      if (status === SUCCESS_TASK_FORM_STATUS) {
        props.openNotification(
          NOTIFICATION_SUCCESS_STATUS,
          SUCCESS_CREATE_TASK_TITLE,
          "Задание было успешно создано.",
        );
        props.setShow(false);
        setTimeout(() => {
          dispatch(setUpdatingTask({}));
  ...
  return (
    <div className="header-form">
      ...
      <div
        className={
          props.isCreateTaskForm ? "header-info" : "header-journal-info"
        }
      >
        ...
        <span className="header-text">
          {props.isCreateTaskForm ? TASK_FORM_TITLE_TEXT : journalGroupName}
        </span>
      </div>
      {props.isCreateTaskForm && (
        <div className="create-wrap">
          <div className="create-btn-wrap">
            ...
            >
              <span className="create-task-btn-text">{buttonText}</span>
            </Button>
          </div>
        </div>
      )}
