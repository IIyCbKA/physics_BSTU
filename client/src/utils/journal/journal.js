...
export default function Journal(props) {
  ...
  const decorWrapStyle = () => {
    return {
      visibility: props.isLoad + isHover + isMobile ? "visible" : "hidden",
    };
  };
    ...
    >
      <div className="journal-main" style={mainStyle()} onClick={journalClick}>
        ...
        <div className="journal-title-wrap">
          <div className="journal-title-text">{props.groupName}</div>
        </div>
        <div
        ...
        >
          {props.isLoad ? (
            <CircularProgress
              style={{ width: "24px", height: "24px", color: "#161616" }}
            />
          ) : (
            <KeyboardArrowRight
              style={{ fontSize: "24px", color: "#161616" }}
            />
          )}
        </div>
      </div>
