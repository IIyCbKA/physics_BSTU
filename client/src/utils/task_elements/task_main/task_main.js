...
export default function TaskMain(props) {
  ...
  const moreWrapStyle = () => {
    const visibility = {
      visibility: props.isHover + props.open + isMobile ? "visible" : "hidden",
    };
    if (props.open) {
  ...
  const mainStyle = () => {
    if (props.isActive) {
      return { borderBottom: "none", backgroundColor: "#E8F0FE" };
  ...
  return (
    <div className="task-main" style={mainStyle()} onClick={handleTaskClick}>
      <div className="task-icon-wrap">
        <AssignmentOutlined style={styles.iconStyle} />
      </div>
      <div
        className={
          props.isTask ? "task-title-wrap" : "task-title-wrap work-title-wrap"
        }
      >
        <div className="task-title-text task-main-text-all">{props.title}</div>
      </div>
      {props.isWork && (
